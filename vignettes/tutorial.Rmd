...
### Longitudinal Business Survey (part 3)

We may also be interested in assessing the estimate of a ratio. In the ELE survey, wage productivity is estimated as the ratio between value added and the total gross remuneration of personnel hired by the company. The `create_prop` function includes a `denominator` parameter, where the variable to be used as the denominator in the ratio should be specified.
...
The ratio estimation is available for ECLAC 2023 standard (`eclac_2023`). If we want to include the ECLAC inputs, we must to add the `scheme` parameter, wich allows to specify the methodological standard to use. By default, the function uses the ECLAC 2020 standard (`eclac_2020`), which does not support ratio estimation.


```{r, eval=FALSE}

create_prop('VA_2022f', denominator = 'REMP_TOTAL', domains = 'cod_actividad+ cod_tamano', design = dc_ele, 
...
eval_ratio <- assess(prod_salarial, scheme = 'chile_economicas', domain_info = TRUE, table_n_obj = table_n_obj, ratio_between_0_1 = FALSE)
