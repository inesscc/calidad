...
get_mean <- function(vars, design, na.rm=FALSE, deff=FALSE,...){
  ...
  ## verificar que sea un disenio del paquete survey
  ...
  x <- sweep(x, 2, mu_hat) # Centrar x por columnas restando la media (residuos x_i -\hat(mu)).
  ...
  estimation <- data.frame(est = mu_hat, se = sqrt(v)) %>%
    dplyr::mutate(cv = se/est)  ## add cv

  ## get deff
  if(deff){

    nobs<-NROW(design$cluster[[1]])
    #vsrs<-variance_MAS(x, design, na.rm=na.rm)/nobs
...
linearization_variance <- function(ui, strata, psu, fpc=NULL, lonely.psu = getOption("survey.lonely.psu"), nPSU = NULL){
  ...
  df_psu <- df %>%
    dplyr::group_by(strata, psu) %>%
    dplyr::summarise(E_hi = sum(ei), .groups = "drop")


  # nPSU observadas por estrato en el subset actual
  nh_obs <- df_psu %>%
    dplyr::count(strata, name = "nh_obs")
  ...
  df_npsu <- df_npsu %>%
    dplyr::mutate(dropped = pmax(nPSU - nh_obs, 0L))

  if(sum(df_npsu$dropped)>0){

    zeros <- df_npsu %>%
      dplyr::filter(dropped > 0) %>%
      tidyr::uncount(dropped, .remove = FALSE) %>%
  ...
  var_by_stratum <- df_psu %>%
    ...
    dplyr::summarise(
      nPSU = unique(nPSU),
      nh_obs = unique(nh_obs),
      mean_E = sum(E_hi)/ nPSU,      # mean(E_hi),
      ss = sum((E_hi - mean_E)^2),   # centramos
      .groups = "drop"
    ) %>%
    dplyr::mutate(
      nh_obs = nh_obs,
  ...
  if (length(lonely_idx) > 0) {
    ...
    # var_by_stratum$df[lonely_idx] <- 1
    ...
    if (lonely.psu == "adjust") {
      ...
      # tratamos los lonely como si fueran certainty: var = 0
      var_by_stratum$ss[lonely_idx] <- 0

    }# else if (lonely.psu == "average") {
    #   # s2 con los no-lonely para obtener la varianza media
    #   tmp_s2 <- var_by_stratum$ss * var_by_stratum$df
    #   s2_mean <- mean(tmp_s2[!var_by_stratum$lonely & !var_by_stratum$certainty])
    #
    #   var_by_stratum$ss[lonely_idx] <- s2_mean / var_by_stratum$df[lonely_idx]
    # }
  }


  var_by_stratum <- var_by_stratum %>%
    dplyr::mutate(s2 = ss* df)

  ## verificamos correccion por poblacion finita
  if(!is.null(fpc)){
    var_by_stratum <- var_by_stratum %>%
      dplyr::left_join(fpc, by = 'strata') %>%
      dplyr::mutate(fpc = ifelse(nPSU/Nh == 1, 0, (1-(nPSU/Nh))),
                    s2 = s2 * fpc)
  }
...
get_FUN_domain <- function(vars, denominator= NULL, design, fun_est, domains=NULL, na.rm=FALSE, deff=FALSE){
  ...
  if(!is.null(domains)){
    ...
    design$nPSU <- dplyr::tibble(strata = design$strata[[1]],
      ...
      dplyr::group_by(strata) %>%
      dplyr::reframe(nPSU = dplyr::n_distinct(cluster))
